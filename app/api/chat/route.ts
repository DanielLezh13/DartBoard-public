...
export async function POST(request: NextRequest) {
  try {
    ...
    // Build complete system message using the unified prompt assembly pipeline.
    // This centralizes all prompt assembly in the correct priority order:
    // 1. System Prompt Kernel (runtime rules)
    ...
    const systemMessageContent = await buildSystemPrompt({
      mode,
      modelId: model,
