...
function resolveDbPath(configPath: string | undefined): string {
  if (configPath) {
  ...
  // Railway volumes are commonly mounted at /data.
  // If DB_PATH is missing in production but /data exists, prefer persistent storage.
  if (process.env.NODE_ENV === "production" && fs.existsSync("/data")) {
    return "/data/dartz_memory.db";
  }

  return path.join(process.cwd(), "dartz_memory.db");
}

const dbPath = resolveDbPath(configuredDbPath);
...
export function getDb(): Database.Database {
  ...
  if (process.env.NODE_ENV === "production") {
    ...
    if (!configuredDbPath) {
      console.warn(
        "[DB] DB_PATH is not set. Set DB_PATH to your persistent volume mount path (example: /data/dartz_memory.db)."
      );
    } else if (!path.isAbsolute(configuredDbPath)) {
      console.warn(
        `[DB] DB_PATH is relative (${configuredDbPath}). In production this may resolve to ephemeral app storage. Prefer an absolute mounted-volume path.`
      );
    }
    if (dbPath.startsWith(process.cwd())) {
      console.warn(
        `[DB] SQLite path is inside app dir (${dbPath}). On Railway deploys this is typically ephemeral and can reset data.`
      );
    }
  }
  ...
  db.exec(`
    ...
    CREATE INDEX IF NOT EXISTS idx_archive_import_events_user_time
      ON archive_import_events(user_id, created_at_ms);
  `);
  ...
  // Migration: ensure archive_import_events.bytes_uploaded exists for byte-based rate limiting
  try {
    const importEventCols = db
      .prepare("PRAGMA table_info(archive_import_events)")
      .all() as Array<{ name: string }>;
    const hasBytesUploaded = importEventCols.some((c) => c.name === "bytes_uploaded");
    if (!hasBytesUploaded) {
      db.prepare(
        "ALTER TABLE archive_import_events ADD COLUMN bytes_uploaded INTEGER NOT NULL DEFAULT 0"
      ).run();
    }

    db.prepare(
      "UPDATE archive_import_events SET bytes_uploaded = 0 WHERE bytes_uploaded IS NULL"
    ).run();
  } catch {
    // ignore
  }
  ...
  // Migration: ensure user_profile.user_id exists for per-user plans/profile lookups.
  // Needed before getUserPlan() queries user_profile by user_id.
  try {
    db.prepare(`ALTER TABLE user_profile ADD COLUMN user_id TEXT`).run();
  } catch (e: any) {
    if (!e.message?.includes("duplicate column")) {
      console.warn("Error adding user_id column to user_profile (may already exist):", e);
    }
  }
  try {
    db.exec(`CREATE INDEX IF NOT EXISTS idx_user_profile_user_id ON user_profile(user_id)`);
  } catch (e: any) {
    console.warn("Error creating user_profile.user_id index:", e);
  }
