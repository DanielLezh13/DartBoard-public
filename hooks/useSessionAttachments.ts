...
export function useSessionAttachments(
  ...
  const hydrate = useCallback(async (sessionId: number) => {
    ...
    devLog("[SESSION_ATTACHMENTS_REQ]", { url, scopeKind: "inferred", hasGuestHeader: !!(headers as Record<string, string>)["x-guest-id"] });
    try {
  ...
  const togglePin = useCallback(async (sessionId: number, memoryId: number) => {
    devLog("[PIN_TOGGLE_CALL]", { sessionId, memoryId });
    devLog("[PIN_TOGGLE] Request:", { sessionId, memoryId });
    try {
      ...
      devLog("[PIN_TOGGLE] Response:", result);
