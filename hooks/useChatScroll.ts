...
export function useChatScroll(opts: { isSending: boolean; isSearching: boolean | React.MutableRefObject<boolean> }) {
  ...
  useEffect(() => {
    ...
    const onWheel = (e: WheelEvent) => {
      ...
      if ((revealMessageIdRef.current !== null || isSending) && e.deltaY < 0) {
    ...
    const handleScroll = () => {
      ...
      if ((revealMessageIdRef.current !== null || isSending) && !programmaticScrollRef.current) {
        if (userScrolledUp) {
          userScrolledAwayDuringStreamRef.current = true;
          userStoppedFollowRef.current = true;
          isFollowingRevealRef.current = false;
      ...
      if ((revealMessageIdRef.current !== null || isSending) && userScrolledDown) {
