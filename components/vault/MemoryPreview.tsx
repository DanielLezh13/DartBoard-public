...
/**
 * Walk the editor document and assign equal column widths to any table
 * whose cells lack `colwidth` attributes (e.g. tables hydrated from markdown).
 * Runs once after setContent so the NodeView picks up explicit widths and
 * switches to `table-layout: fixed`.
 *
 * Width seeding uses each table's rendered wrapper width (not the global
 * editor width), so nested tables (e.g. inside list items) start at the same
 * wall they will be clamped to during drag.
 */
  ...
  doc.descendants((node: any, pos: number) => {
    ...
    for (let r = 0; r < node.childCount; r++) {
      ...
      for (let c = 0; c < row.childCount; c++) {
        ...
        tr.setNodeMarkup(cellPos, undefined, {
          ...cell.attrs,
          colwidth: new Array(colspan).fill(colWidth),
        });
        modified = true;
...
export default function MemoryPreview({
  ...
  const memoryKey = useMemo(() => {
    const id = (memory as any)?.id ?? null;
    const sessionId = (memory as any)?.session_id ?? null;
    const messageId = (memory as any)?.message_id ?? null;
  ...
  useEffect(() => {
    if (memory) {
      ...
      setEditedExcerpt(memory.excerpt !== null && memory.excerpt !== undefined ? memory.excerpt : "");

      const isDraftHydrate = !!(isDraft || (memory as any)?.id === -1);
      if (isDraftHydrate) {
        // For drafts, use folder_id as single source of truth
        const folderId = (memory as any)?.folder_id ?? null;
        // Derive display name from folder_id lookup
        const folderLabel = folderId && folderObjects 
          ? folderObjects.find(f => f.id === folderId)?.name ?? "Unsorted"
          : "Unsorted";
  ...
  useEffect(() => {
    ...
    const folderId = (isDraft || memory?.id === -1)
      ? ((memory as any)?.folder_id ?? null)
  ...
  }, [folders, isDraft, memory?.id, (memory as any)?.folder_id, folderObjects]);

  // Initialize draft state when draft mode is enabled
  useEffect(() => {
    if (isDraft && memory) {
      const incomingTitle = memory.title || "";
  ...
  }, [isDraft, memory?.id, memory?.title, memory?.summary, hasEditedTitle, isEditing]);
  ...
  useEffect(() => {
    ...
    Promise.resolve(marked.parse(memory.summary))
      .then((html) => {
      ...
      .catch((err) => {
        ...
        if (editor && !editor.isDestroyed) {
          editor.commands.setContent(`<p>${(memory.summary || "").replace(/\n/g, "<br>")}</p>`, false);
        }
      });
  }, [editor, memory?.id, memory?.summary, memoryDocJson]);

  // Handle forced edit mode (e.g., when creating a new memory or opening draft)
  useEffect(() => {
    if (isDraft || (forceEditMemoryId && memory && memory.id !== undefined && memory.id === forceEditMemoryId)) {
      setIsEditing(true);
  ...
  }, [forceEditMemoryId, memory?.id, isDraft, editor, onEditHandled]);
