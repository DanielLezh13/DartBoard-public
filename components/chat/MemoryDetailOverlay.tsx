...
export function MemoryDetailOverlay({
  ...
  const latestMemoryRef = React.useRef<Memory | null>(memory);
  const latestDraftMemoryRef = React.useRef<DraftMemory | null>(draftMemory);
  ...
  React.useEffect(() => {
    latestDraftMemoryRef.current = draftMemory;
  }, [draftMemory]);
  ...
  React.useEffect(() => {
    if (open) {
      setMounted(true);
      setVisible(false);
      const latestMemory = latestMemoryRef.current;
      const latestDraftMemory = latestDraftMemoryRef.current;
      ...
      if (latestMemory) {
        setDisplayedMemory(latestMemory);
        setDisplayedDraft(null);
      } else if (latestDraftMemory) {
        setDisplayedDraft(latestDraftMemory);
  ...
  }, [draftMemory, draftMemory?.title, open, mounted, displayedDraft, displayedDraft?.summary, displayedDraft?.title]);
