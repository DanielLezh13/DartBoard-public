...
import { SessionFolderRail, FolderBubbleIconContent, type Folder as SidebarFolder, type Session as SidebarSession } from "@/components/chat/SessionFolderRail";
import { DefaultFolderIcon } from "@/components/icons/DefaultFolderIcon";
import { MemoryDock } from "@/components/chat/MemoryDock";
import { MemoryDetailOverlay } from "@/components/chat/MemoryDetailOverlay";
...
export function ChatPageLayout(props: any) {
  ...
  // Folder appearance overrides (from SessionFolderRail) for left-rail DragOverlay icon
  const [leftFolderAppearance, setLeftFolderAppearance] = useState<Record<number, { label?: string; icon?: string; color?: string }>>({});
  // Folder appearance overrides (from MemoryFolderRail) for right-rail memory folder DragOverlay icon
  ...
  const openMemoryDetailOverlay = (memoryId: number) => {
    if (draftMemory) {
      setDraftMemory(null);
    }
  ...
  const resolvedOverlayMemory = React.useMemo(() => {
    if (draftMemory) return null;
    const selId = selectedMemoryId == null ? null : Number(selectedMemoryId);
  ...
  }, [draftMemory, selectedMemoryId, getMemoryById, memories]);
  ...
  const [landingLogoSlidePx, setLandingLogoSlidePx] = React.useState(204);

  React.useEffect(() => {
    if (typeof window === "undefined") return;
    const isWindows = /Windows/i.test(window.navigator.userAgent || "");
    setLandingLogoSlidePx(isWindows ? 210 : 204);
  }, []);
  ...
  React.useEffect(() => {
    if (typeof window === "undefined") return;
    let cancelled = false;
    const warmScope = scope;
    const warmProfileChunk = async () => {
      const profileModule = await import("@/components/profile/ProfileView");
      if (cancelled) return;
      if (warmScope?.kind === "user" && typeof profileModule.prefetchProfileForScope === "function") {
        void profileModule.prefetchProfileForScope(warmScope as any);
      }
    };
  ...
  React.useEffect(() => {
    if (typeof window === "undefined") return;
    // Message contract used by /home iframe embed actions.
    // Producers: HeroSection handleEmbeddedChat/handleEmbeddedArchive.
  ...
  React.useEffect(() => {
    ...
    return () => cancelAnimationFrame(id);
  }, [effectiveIsLanding, isResuming, uiHydrating]);
      ...
      >
        ...
        {(() => {
          ...
          <div className="min-w-0 overflow-x-hidden overflow-y-hidden md:block">
            ...
            >
              {/* Drawer wrapper: keep mounted; animate width like MemoryDock for smoother parity */}
              <div
                className={
            ...
            >
              ...
              {memoryOverlayOpen ? (
                <div className="relative min-w-0 flex-1 flex items-center justify-center">
                  ...
                  >
                    {(() => {
                      const m: any = resolvedOverlayMemory;
                    ...
                    >
                      <span className="relative block h-7 w-7">
                        {isAuthenticated && canRenderAvatarImage && (
                          // eslint-disable-next-line @next/next/no-img-element -- external avatar URLs come from auth profile metadata
                          <img
                            src={avatarUrl}
                            alt="Profile"
          ...
          />
            ...
            >
                ...
                <div
                  className={
                    landingFadeOut && !forceLanding
                      ? "absolute" // freeze position; inherit opacity from parent
                  ...
                  style={
                    landingFadeOut && !forceLanding
                      ? {
                          transform: `translateX(${effectiveStageForRender >= 2 ? -landingLogoSlidePx : 0}px)`,
                          transition: "none",
                          willChange: "auto",
                        }
                      : {
                  ...
                  <FancyGlowingModeButton
                    onClick={() => {
                      ...
                      if (process.env.NODE_ENV === "development") {
                        devLog("[MODE] cycle", { mode, currentIndex, safeIndex, nextIndex, nextMode });
                      }
              ...
              <LandingInjectedMemories
                ...
                onOpenAttachedMemory={openMemoryDetailOverlay}
            ...
            <MemoryDetailOverlay
              ...
              onSaveDraft={async (draft: DraftMemory) => {
                devLog("[DRAFT SAVE] keys:", Object.keys(draft));
                devLog("[DRAFT SAVE] full draft:", draft);
                devLog("[DRAFT SAVE] incoming draft", draft);
                ...
                devLog("[DRAFT SAVE] payload with folder_id:", payload);
                devLog("[DRAFT SAVE] folder_id being sent:", payload.folder_id);
                  ...
                  if (!response.ok) {
                    ...
                    throw new Error(errorMessage);
	                  ...
	                  devLog("[DRAFT SAVE] server returned", newMemory);
	                  const topPos = -(Date.now() * 1000 + Math.floor(Math.random() * 1000));
	                  const normalizedId = upsertMemoryInCache?.(newMemory as any) ?? Number((newMemory as any)?.id);
	                  if (!Number.isFinite(normalizedId)) {
	                    throw new Error("Failed to resolve saved memory id");
          ...
          {/* Column 4: MemoryFolderRail */}
          <div className="block bg-transparent pointer-events-none" aria-hidden="true">
            {/* Rail is structural only - no buttons */}
          </div>

          {/* Column 5: MemoryPanel */}
          <div className="min-w-0 overflow-x-hidden overflow-y-hidden lg:block">
            ...
            onToggleMemoryDock={toggleRight}
            memoryPanelOpen={rightPanelOpen}
            activeSessionId={activeSessionId}
          ...
          />
          {/* Memory + memory-folder drag overlay: portal to body so it always layers above the chat drop target. */}
            ...
            ? createPortal(
                <DragOverlay adjustScale={false} dropAnimation={null} zIndex={120}>
                  {activeDragId?.startsWith("memory-folder-") ? (
                    ...
                    (() => {
                      if (process.env.NODE_ENV === "development") {
                        devLog("[DND] Memory DragOverlay render (desktop)", { dragOverlayMemoryId });
                      }
        ...
        {(layoutMode === "narrow" || keepOverlaysVisible) && (
          <>
            ...
            <div
              className="fixed top-0 left-0 z-[60] h-full w-[329px] pointer-events-none"
              style={{
            ...
            <div
              className="fixed top-0 right-0 z-[60] h-full w-[329px] pointer-events-none"
              style={{
                ...
                <div className="h-full w-[329px] bg-gray-900 pointer-events-auto overflow-x-hidden overflow-y-hidden shadow-[-12px_0_28px_rgba(0,0,0,0.45),-4px_0_10px_rgba(0,0,0,0.35)] ring-1 ring-white/5">
                  ...
                  >
                    ...
                    <MemoryDock
                      ...
                      onToggle={() => setRightPanelOpen((v: any) => !v)}
                      onToggleMemoryDock={() => closeOverlays()}
                      memoryPanelOpen={rightPanelOpen}
                      activeSessionId={activeSessionId}
                      folderListContainerRef={rightFolderListRef}
                      folders={memoryFolders}
                      ...
                      ? createPortal(
                          <DragOverlay adjustScale={false} dropAnimation={null} zIndex={120}>
                            {activeDragId?.startsWith("memory-folder-") ? (
                              ...
                              (() => {
                                if (process.env.NODE_ENV === "development") {
                                  devLog("[DND] Memory DragOverlay render (narrow)", { dragOverlayMemoryId });
                                }
                                const memory = memories.find((m: any) => m.id === dragOverlayMemoryId);
                                if (!memory) return null;
                                const title = memory.title || "Untitled";
        ...
        {profileOverlayOpen && (
          <div className="fixed inset-0 z-[130]">
            ...
            />
            <div
              className={`absolute inset-0 overflow-y-auto overscroll-contain transition-[opacity,transform] duration-200 ease-out ${
